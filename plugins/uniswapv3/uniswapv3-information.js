...
const getUniswapV3TokenPrice = async (tokenAddress0, tokenAddress1, poolAddress, settings, networkName) => {
...
const getUniswapV3AmountOut = async (tokenAddress0, tokenAddress1, amountIn, settings, networkName, poolPercentage = '10000') => {
    const provider = new ethers.providers.JsonRpcProvider({
        url: settings.environement['rpc' + networkName].url,
        name: settings.environement['rpc' + networkName].name,
        chainId: Number(settings.environement['rpc' + networkName].chainId)
    ...
    const poolAddress = settings.poolAddress ?? await factoryV3Contract.getPool(tokenAddress0, tokenAddress1, poolPercentage);
    const poolContract = new ethers.Contract( // Uniswap v3 ABI
        poolAddress,
        [
            { "inputs": [], "name": "liquidity", "outputs": [ { "internalType": "uint128", "name": "", "type": "uint128" } ], "stateMutability": "view", "type": "function" },
